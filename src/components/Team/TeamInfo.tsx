...
const teamMembers = [
  ...
  },
  {
    ...
    description: "Our development team has a combined 20 years experience in full stack development. Including an 8 year head of IT and multiple solidity and commercial deployments.",
    image: tomImage,
    icons: [xIcon, telegramIcon, linkedinIcon],
...
const TeamInfo: React.FC = () => {
  ...
  const nextPage = () => {
    setCurrentPage((prevPage) => (prevPage + 1) % Math.ceil(teamMembers.length / itemsPerPage));
  };

  const prevPage = () => {
    setCurrentPage((prevPage) =>
      prevPage === 0 ? Math.ceil(teamMembers.length / itemsPerPage) - 1 : prevPage - 1
    );
  };

  // Slice the array to show only 2 items per page
  const visibleMembers = teamMembers.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage);
  ...
  return (
    <div className={styles.carouselContainer}>
      ...
      <div className={styles.carouselControls}>
        <button onClick={prevPage} className={styles.arrowButton}>←</button>
        <span>{`${currentPage + 1}/${Math.ceil(teamMembers.length / itemsPerPage)}`}</span>
        <button onClick={nextPage} className={styles.arrowButton}>→</button>
      </div>
    </div>
  );
};
